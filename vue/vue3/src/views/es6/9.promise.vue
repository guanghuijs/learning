...
{
  const loadImageAsync = (url:string) => new Promise((resolve, reject) => {
    const img = new Image();
    img.onload = () => {
      resolve(img);
    };
  ...
  onMounted(() => {
    loadImageAsync('http://img.gaosan.com/upload/201909/20190920085545319.jpg').then((res) => {
...

{
  ...
  xhr.open('GET', 'https://www.fastmock.site/mock/961b619357977ecee63001f0f5140734/yghh/phoneUser');
  // 如果是post请求  必须要写请求头
  ...
  const promiseAjax = (options:{
    ...
    console.log(options);
    promiseXhr.open(options.type || 'get', options.url, options.async || true);
    promiseXhr.send();
  ...
  promiseAjax({ url: 'https://www.fastmock.site/mock/961b619357977ecee63001f0f5140734/yghh/svperImg' }).then((res) => {
    console.log(res);
  });
}
